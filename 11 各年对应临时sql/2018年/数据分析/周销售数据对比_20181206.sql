select case   
         when t1.order_day >= to_date('2018-11-26', 'yyyy-mm-dd') and
              t1.order_day <= to_date('2018-11-28', 'yyyy-mm-dd') then
          '上周'       
         when t1.order_day >= to_date('2018-12-03', 'yyyy-mm-dd') and
              t1.order_day <= to_date('2018-12-05', 'yyyy-mm-dd') then
          '本周'
       end 周类型, 
       case
 ...
 where t2.flag <> 2
   ...
   and to_char(t1.order_day,'day') in('星期一','星期二','星期三')
 group by case   
         when t1.order_day >= to_date('2018-11-26', 'yyyy-mm-dd') and
              t1.order_day <= to_date('2018-11-28', 'yyyy-mm-dd') then
          '上周'       
