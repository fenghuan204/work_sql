select /*+parallel(8) */
h1.channel,h1.flights_segment_name,h1.aheady,h1.passtype,h1.ttype2,h1.is_swj,h1.is_rcd,h1.去程间隔,h1.返程间隔,
...
select t1.flights_order_id,t1.flights_order_head_id,t1.wf_lc_father_id,t2.flights_segment_name,
...
case when nvl(t1.min_seat_name,t1.seats_name)=t1.seats_name then '否'
     ...
     CASE WHEN (CASE WHEN t3.nationality ='CHN' THEN '中国'
      ...
      CASE WHEN (CASE WHEN t3.nationality ='CHN' THEN '中国'
          ...
          lead(t1.flights_date,1)over(partition by t1.wf_lc_father_id order by t1.flights_date)-t1.flights_date 去程间隔,
           t1.flights_date-lag(t1.flights_date,1)over(partition by t1.wf_lc_father_id order by t1.flights_date) 返程间隔,                
 ...
 group by t1.flights_order_id,t1.flights_order_head_id,t1.wf_lc_father_id,decode(t1.sex,1,1,2) ,t2.flights_segment_name,
t1.channel,t1.segment_head_id,case when t1.ahead_days<=7 then '0~7' 
when t1.ahead_days<=30 then '8~30'
when t1.ahead_days<=60 then '31~60'
when t1.ahead_days<=90 then '61~90'
     ...
     CASE WHEN (CASE WHEN t3.nationality ='CHN' THEN '中国'
      ...
      CASE WHEN (CASE WHEN t3.nationality ='CHN' THEN '中国'
          ...
          CASE WHEN t8.province IS NOT NULL THEN t8.city
               WHEN t8.province IS NULL AND tt1.province IS NOT NULL THEN tt1.city
               ELSE NULL END
          ELSE NULL END,t1.flights_date
     )h1
          group by h1.channel,h1.aheady,h1.passtype,h1.ttype2,h1.is_swj,
h1.is_agent,case when age<12 then '<12'
            when age<18 then '12~17'
            when age<24 then '18~23'
            when age<30 then '24~29'
           ...
           when h1.ticket< h1.num3 and   h1.flights_order_head_id<>h1.wf_lc_father_id  then '返程'
           else '单程' end,h1.去程间隔,h1.返程间隔,h1.flights_segment_name
