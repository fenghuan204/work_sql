...
as
select mid1.whole_flight,mid1.flightno,mid1.flights_segment_name,mid0.segment_code,mid1.round_time/60 round_time,
mh.cw_hourvaricost,mh.mh_hourvaricost,
case when mh.cw_hourvaricost >0 then mh.cw_hourvaricost
else 
case when mid2.vari_cost is not null or mid3.vari_cost is not null
or mid4.vari_cost is not null 
then nvl(nvl(mid2.vari_cost,mid3.vari_cost),mid4.vari_cost)/nvl(nvl(mid2.round_time,mid3.round_time),mid4.round_time)
when sc1.flights_city_name is not null or sc1.flights_city_name is not null
then nvl(sc1.varicosthour,sc1.varicosthour)
when third1.originairport_name is not null or third2.originairport_name is not null
then nvl(third1.varicosthour,third2.varicosthour)
when four.income_type is not null
then  four.varicosthour
when six.nationflag is not null
then  six.varicosthour
else five.varicosthour
end
end varicosthour,

case when mh.cw_hourvaricost >0 then mh.cw_hourvaricost*(mid1.round_time/60) 
else 
case when mid2.vari_cost is not null or mid3.vari_cost is not null
...
mh.cw_hourvaricost*(mid1.round_time/60) cost0,
...
--财务提供变动成本及上报民航局
left join hdb.rep_dim_varicost mh on mid1.whole_flight=mh.whole_flight and mid0.segment_code=mh.flights_segment
