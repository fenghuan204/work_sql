   ...
   select to_char(cp.createtime, 'yyyymm') 月份,
       ...
       case when cp.complaintype1c like '%退改%' then '机票退改签'
       when cp.complaintype2c like '%退改%' then '机票退改签'
       when cp.complaintype3c like '%退改%' then '机票退改签'
       when cp.conetent like '%退改%' then '机票退改签'
       when cp.complaintype1c like '%退款%' then '机票退改签'
       when cp.complaintype2c like '%退款%' then '机票退改签'
       when cp.complaintype3c like '%退款%' then '机票退改签'
       when cp.conetent like '%退款%' then '机票退改签'
       when cp.complaintype1c like '%改签%' then '机票退改签'
       when cp.complaintype2c like '%改签%' then '机票退改签'
       when cp.complaintype3c like '%改签%' then '机票退改签'
       when cp.conetent like '%改签%' then '机票退改签'
       when cp.complaintype1c like '%退票%' then '机票退改签'
       when cp.complaintype2c like '%退票%' then '机票退改签'
       when cp.complaintype3c like '%退票%' then '机票退改签'
       when cp.conetent like '%退票%' then '机票退改签'              
       end 是否退改签,             
       cp.sex 性别,
       cp.age 年龄,
...
/*   or cp.conetent like '%退%'
   ...
   and not regexp_like(nvl(cp.complaintype1c,'-'),'(旅客网上误点退票)|(退改签操作流程)')
